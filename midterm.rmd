...
<code>cal_streamdist</code>, <code>sac_streamdist</code>: For Calgary and Sacramento County respectively, <code>cal_streamdist</code> and <code>sac_streamdist</code> describe the distance between any cell and a stream. <code>cal_riverdist</code> and <code>sac_riverdist</code> also describes the distance the cells are to major rivers. These variables come to mind considering our hypothesis that areas with streams have a higher flooding susceptibility. 
...
#st_transform(cal_residcover, crs=3780)
crs(cal_residcover) = "EPSG:3780" # set CRS
st_crs(cal_residcover)
transformed_dataframe <- st_transform(cal_residcover, crs = 3780)
...
cal_riverdist = cal_riverdist %>% select(LastTableCalgary.MEAN)
...
allCalgary = st_join(cal_dem, cal_streamdist, largest = TRUE) %>%
             ...
             st_join(cal_riverdist, largest = TRUE) %>% 
  st_join(cal_residcover, largest = TRUE) #%>%
  #st_join(cal_treecover, largest = TRUE) %>%
  #st_join(cal_mixedcover, largest = TRUE) %>%
  #st_join(cal_commecover, largest = TRUE) %>%
  #st_join(cal_induscover, largest = TRUE) %>%
  #st_join(cal_agricover, largest = TRUE)
             ##st_join(cal_residcover, cal_mixedcover, cal_commecover, cal_induscover, cal_agricover, largest = TRUE)
...
allCalgary = allCalgary %>% rename(meanElevation = ZonalSt_Fishnet1.MEAN,
                                   ...
                                   riverDistance = LastTableCalgary.MEAN)
...
allsacramento = st_join(sac_dem, sac_streamdist, largest = TRUE) %>%
                st_join(sac_riverdist, largest = TRUE) %>%
                st_join(sac_residcover,  largest = TRUE)

ggplot() +
  geom_sf(data=cal_residcover, aes(fill=as.factor(ZonalSt_Calgary1_MAJORITY))) +
  labs(title="cal_res")
