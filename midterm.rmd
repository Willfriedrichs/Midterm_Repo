...
**Motivations:**
The importance is that this model can be used by several local governments to assess their communities for flood risk and respond with efforts to proactively prepare against potential flood damage based upon the likelihoods of outcomes. Specially, this model has the potential to save lives, money and resources on the preventative side for jurisdictions that are alike.

**Model Description:**
The data collected for models would include current movement or flows of streams, and the types of land uses that exist in communities such (residential, commercial, industrial), because they are often related to the likelihood that the land surfaces would be water permeable. Also, data on community features such as the municipal boundaries, major city locations, urban tree cover, and distance to major rivers and streams are included to provide a physical, natural, and hydrological context to the predictions. 

The document walks through the generalized linear model used to predict flood inundation zones first in Calgary, then in takes the same approach to predicting flooding risk areas for Sacramento County, which would be the goal of the municipal FIMPC. Sacramento County is used for demonstration of the model because both areas have flood risk associated with moving inland rivers. Sacramento County is larger than Calgary's area, but the county used for comparison because land use at the Sacramento County level is more analogous to Calgary land use patterns than land use within Sacramento City proper is. For both Calgary and Sacramento County, predictions will estimate whether a given 500m by 500m "cell" of area can be described as in a flood inundation zone or at risk of flooding.

## 2) Building Variables and Running Model

### Variable descriptions

The following factors will be examined as predictors of Calgary's flood inundation zones. When data are loaded into the model from a specific city, a prefix is attached (e.g. <code>cal_dem</code> for Calgary or <code>sac_dem</code> for Sacramento).

<code>dem</code>: This factor represents elevation data relative to the lowest point in the area. It comes from the Digital Elevation Model (DEM). Flood inundation is probably more likely in low-lying areas than high elevation areas, so there is reason to suspect this factor might be significant.

<code>streamdist</code>: This describes the distance between any cell and a stream. Stream locations are calculated from DEM files using ArcGIS _Flow Direction_ and _Flow Accumulation_ tools, which mark DEM locations with especially high drainage as a stream.

<code>riverdist</code>: This describes the distance the cells are to major rivers. Distance is measured in relation to the closest part of a river's edge rather than in relation to a river's center point.

<codecal_treecover</code>: This factor used to describe the tree cover within an area. The presence of tree density may play a factor in water absorption as well as soil permeability.
...
Calgary and Sacramento County land uses are organized into five types:

<code>residcover</code>: Residential land uses

<code>mixedcover</code>: Mixed Use or transit orientated development land uses.

<code>commecover</code>: Commercial land uses

<code>induscover</code>: Industrial land uses

<code>agricover</code>: Agricultural and open space land uses


<code>inundation</code>: This is what our model aims to predict. We will train our model on part of Calgary flood inundation fishnet, and test our predictions on another part of it. Finally, we will predict an <code>inundation</code> layer for Sacramento.
...
Some of these variables have a stronger relationship to inundation in Calgary than others--only six variables are used in the final prediction of the dependent variable. Some factors like mixed use and agricultural land cover were not at all significant predictors of inundation zones, and others, such as distance to rivers, were left out due to multicolinearity.


The model created by these six variables will be used to predict inundation zones in other areas, so some variables are adjusted for better regional comparison. For example, elevation will be considered relative to cities' lowest points rather than relative to sea level.
...
Inundation in Calgary is mapped below.

This map of Calgary's Inundation Zones, like the other maps below, are made to fit a fishnet with 0.5km by 0.5km cells. The full, high resolution version of the Calgary Inundation Zone map includes the zones that are too skinny to appear on the fishnet, such as the ones following the Bow River north of its convergence with the Elbow River at the center of the map. The model's use of this fishnet version of the Calgary Inundation Zones map means it will be less likely to accurately predict zones along the northern part of the Bow River. A version of this model using a fishnet with smaller cells would probably perform better in that area. 

```{r ggplot_inundation, warning = FALSE, message = FALSE}
ggplot() +
...
Each of the predictors in Calgary are mapped below.

```{r ggplot_meanElevation, warning = FALSE, message = FALSE}
ggplot() +
...
```{r ggplot_meanStreamDistance, warning = FALSE, message = FALSE}
ggplot() +
...
```{r ggplot_residcover, warning = FALSE, message = FALSE}
ggplot() +
...
```{r ggplot_commecover, warning = FALSE, message = FALSE}
ggplot() +
...
```{r ggplot_induscover, warning = FALSE, message = FALSE}
ggplot() +
...
The AUC (Area Under Curve) measures the model's ability to make distinctions between the two classes, in this case non-inundation and inundation zones on a fishnet. An AUC close to 1.0 suggests that the model is a relatively well-performing one.
...
Spatial cross validation creates regional test sets, such that any region within the fishnet where the function performs particularly poorly can be identified. With the help of the <code>mlr</code> package, a learner is created that automatically partitions the Calgary fishnet into five regional folds. This folding process is repeated 100 times. Summary statistics of the accuracy of predictions based on the folds are displayed below. These results, similar to those above, confirm the model's accuracy.
...
## 4) Calgary and Sacramento Full Predictions

### Calgary Confusion Matrix Map

The map below shows confusion matrix values by cell across Calgary based on the model. While the model generates a likelihood that a cell falls into an inundation zone or not, for the purposes of the following analysis, any prediction of greater than 0.15 will be considered a prediction of an inundation zone. 

True positives start around the confluence of the Bow and Elbow rivers, which is visible as the purple area near the centroid of Calgary's shape. The true positives follow the Bow river south until it reaches the city limits. False negatives occur along the Elbow River towards the west, and in isolated areas of the Bow river north of the previously mentioned confluence point. False positives appear sporadically near streams and low-lying areas.
...
### Full Calgary Prediction Map

Below are the raw results of model's predictions on Calgary as a whole. The predictions illustrated below are the basis for the confusion matrix analysis above. Rather than being presented as a binary, the map below shows probabilities of an inundation zone as estimated by the model as a decimal.
...
Section 2 of this analysis references issues associated with the fishnet cell size. In retrospect, using a fishnet of inundation zones with smaller cell sizes and making predictions accordingly may have resulted in a more accurate set of predictions. 

### Full Sacramento Prediction Map

```{r full_sac_prediciton, warning = FALSE, message = FALSE}
...
Each of the predictors in Sactown are mapped below.

```{r ggplot_meanElevation_sac, warning = FALSE, message = FALSE}
ggplot() +
  geom_sf(data=allSacramento, aes(fill=as.numeric(meanElevation)), color=NA) +
  scale_fill_viridis() +
  labs(title="Mean Elevation (relative to lowest point)", fill="Elevation (m)") +
  mapTheme
```

```{r ggplot_meanStreamDistance_sac, warning = FALSE, message = FALSE}
ggplot() +
  geom_sf(data=allSacramento, aes(fill=as.numeric(meanStreamDistance)), color=NA) +
  scale_fill_viridis() +
  labs(title="Mean Distance to Nearest Stream", fill="Distance (m)") +
  mapTheme
```

```{r ggplot_residcover_sac, warning = FALSE, message = FALSE}
ggplot() +
  geom_sf(data=allSacramento, aes(fill=as.factor(residcover)), color=NA) +
  labs(title="Residential Land Use", fill="Land Use Type") +
  scale_fill_manual(labels = c("Non-Residential", "Residential"),
                    values = c("springgreen3","seagreen")) +
  mapTheme
...
```{r ggplot_commecover_sac, warning = FALSE, message = FALSE}
ggplot() +
  geom_sf(data=allSacramento, aes(fill=as.factor(commecover)), color=NA) +
  labs(title="Commercial Land Use", fill="Land Use Type") +
  scale_fill_manual(labels = c("Non-Commercial", "Commercial"),
                    values = c("springgreen3","turquoise1")) +
  mapTheme
...
```{r ggplot_induscover_sac, warning = FALSE, message = FALSE}
ggplot() +
  geom_sf(data=allSacramento, aes(fill=as.factor(induscover)), color=NA) +
  labs(title="Industrial Land Use", fill="Land Use Type") +
  scale_fill_manual(labels = c("Non-Industrial", "Industrial"),
                    values = c("springgreen3","yellow3")) +
  mapTheme
