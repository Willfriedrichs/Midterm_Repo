...
library(cowplot)
library(modelr)
install.packages("lwgeom" =TRUE)
```

## 1) Goals, & Motivations, & Model Description 

Goals:
This document serves as supplemental material to the proposal for the creation of municipal Flood Inundation Modeling & Planning Centers (FIMPC). The document provides a predictive 'model' that determines the probability an area floods. The aim is to research risks and to better prepare local residents, property owners, and municipalities to put forward plans for improvements based on the severity of flood active in their area.

Motivations: 
The importance is that this ‘model’ can be used to support the FIMPC and local governments to assess their communities for flood risk and response with efforts to proactively prepare against future potential flood damage based upon the likelihoods of outcomes. 

For example, the FIMPC could use the predictive model to estimate the locations of the most at-risk areas in the context of the worsening storm surges of hurricane season or heavy rain. Planners at FIMPC could then present this data to the city councils, and support resiliency measures given financial constraints via a cost-benefit analysis. The success of such interventions could start a cycle of research, funding, and improvements based on the realm of flood inundation mapping and predictions.

Model Description:
The data collected for this ‘model’ would include the current movement or flow of streams, and the types of land use surfaces that exist in communities such as residential, commercial, agricultural, because they often determine the likelihood that the land surfaces would be water permeable. Also, data on community features such as the municipal boundaries, major city locations, urban tree cover, and distance to major rivers and streams are included to provide a physical, natural, and hydrological context to the predictions. 
...
ggplot() +
  geom_sf(data=inundationAllSacProb, aes(fill=pred, color=pred)) +
  labs(title="Full prediction set for Sacramento") +
  theme(axis.text.x=element_blank(), #remove x axis labels
      axis.ticks.x=element_blank(), #remove x axis ticks
      axis.text.y=element_blank(),  #remove y axis labels
      axis.ticks.y=element_blank()  #remove y axis ticks
